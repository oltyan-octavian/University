...
public class  FriendshipDBRepository implements Repository<Tuple<Long, Long>, Friendship> {
    ...
    public Optional<Friendship> findOne(Tuple<Long, Long> longLongTuple) {
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement("SELECT status," +
                     ...
                     "f.date AS date, " +
                     "id1 as id1, " +
                     "id2 as id2 FROM friendships f INNER JOIN users u1 ON f.id1 = u1.id INNER JOIN users u2 ON f.id2 = u2.id " +
            ...
            if (resultSet.next()) {
                ...
                Friendship prietenie = new Friendship(u1, u2, MyDate, status);
                prietenie.setId(id);
    ...
    public Iterable<Friendship> findAll() {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement("SELECT status, u1.first_name AS u1_first_name, u1.last_name AS u1_last_name, u2.first_name AS u2_first_name, u2.last_name AS u2_last_name, f.date AS date, id1 as id1, id2 as id2 FROM friendships f JOIN users u1 ON f.id1 = u1.id JOIN users u2 ON f.id2 = u2.id" );
        ) {
            ResultSet resultSet = statement.executeQuery();
    ...
    public Optional<Friendship> save(Friendship entity) {
        ...
        try (Connection connection = DriverManager.getConnection(url, username, password);
             PreparedStatement statement = connection.prepareStatement("insert into friendships(id1,id2,date, status) VALUES(?,?,?, ?)")
        ){
    ...
    public Optional<Friendship> update(Friendship entity) {
        try(Connection connection = DriverManager.getConnection(url,username,password);
        PreparedStatement statement = connection.prepareStatement("update friendships set status=? where (id1=? and id2 = ?) or(id2 = ? and id1 = ?)")
        ) {
