...
void testAll(){
    ...
    MDO dictverif = MDO(relatie1);
    dictverif.adauga(1, 2);
    dictverif.adauga(1, 3);
    dictverif.adauga(1, 4);
    dictverif.adauga(2, 3);
    assert(dictverif.dim() == 4);
    vector<TValoare> val1 = dictverif.stergeValoriPentruCheie(1);
    assert(dictverif.dim() == 1);
    assert(val1.size() == 3);
    assert(val1.at(0) == 2);
}
